...
public class Server {
	...
	private void execute() throws IOException {
		...
		while (true) {
			...
			// khi ma ket noi dc 1 cai client noa day , thi them thg day' vao list 
...
class ReadServer extends Thread {
	...
	public void run() {
		try {
			// tao cai luong de doc du lieu 
			DataInputStream dis = new DataInputStream(socket.getInputStream());
			// client gui den thi` client doc du lieu ( lay de lieu ve ) 
			while (true) {
				...
				// doc xong -> gui no di tat ca sever con lai 
